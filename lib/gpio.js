var fs = require('fs');
var Q = require('q');
var readFile = Q.denodeify(fs.readFile);
var writeFile = Q.denodeify(fs.writeFile);
var stat = Q.denodeify(fs.stat);
var logger = require('winston');
var EventEmitter = require('events').EventEmitter;
var util = require('util');
var exec = Q.denodeify(require('child_process').exec);

var exists = function (path) {
   return stat(path)
   .then(function () {
      return true;
   })
   .fail(function () {
      return false;
   });
};

var unexport = function (pin) {
   return writeFile('/sys/class/gpio/unexport', pin);
};

var gpio = {
   'export': function (pin, direction) {
      var pin_path = '/sys/class/gpio/gpio' + pin;
      var that = this;

      return exists(pin_path)
      .then(function (exists) {
         if (exists) {
            return unexport(pin);
         }
      })
      .then(function () {
         return writeFile('/sys/class/gpio/export', pin);
      })
      .then(function () {
         if (direction && direction.test(/^(out|in)$/i)) {
            return writeFile(pin_path + '/direction', direction.toLowerCase());
         } else if (direction) {
            throw 'Unrecognized direction: ' + direction + '. Expected "in" or "out".';
         }
      })
      .then(function () {
         that.emit('export', pin);

         return {
            set: function () {
               that.emit('set', pin);
               return writeFile(pin_path + '/value', '1')
               .then(function () {
                  that.emit('set', pin);
               });
            },
            unset: function () {
               return writeFile(pin_path + '/value', '0')
               .then(function () {
                  that.emit('unset', pin);
               });
            },
            unexport: function () {
               return unexport(pin)
               .then(function () {
                  that.emit('unexport', pin);
               });
            },
            read: function () {
               return readFile('/sys/class/gpio/unexport', pin)
               .then(function (d) {
                  return Number(d.toString()) === 1;
               });
            }
         };
      });
   }
};

if (process.env.USE_PI_GPIO) {
   var gpio = require('pi-gpio');
   var gpio_open = Q.denodeify(gpio.open);
   var gpio_write = Q.denodeify(gpio.write);
   var gpio_read = Q.denodeify(gpio.read);
   var gpio_close = Q.denodeify(gpio.close);

   logger.info('Using PI GPIO');

   gpio = {
      'export': function (pin, direction) {
         var that = this;
         return gpio_open(pin, direction || 'output')
         .then(function () {
            that.emit('export', pin);
            return {
               set: function () {
                  return gpio_write(pin, 1)
                  .then(function () {
                     that.emit('set', pin);
                  });
               },
               unset: function () {
                  return gpio_write(pin, 0)
                  .then(function () {
                     that.emit('unset', pin);
                  });
               },
               unexport: function () {
                  return gpio_close(pin)
                  .then(function () {
                     that.emit('unexport', pin);
                  });
               },
               read: function () {
                  return gpio_read(pin)
                  .then(function (value) {
                     return Number(value) === 1;
                  });
               }
            };
         });
      }
   };
}
else if (process.env.USE_GPIO_BIN) {
   logger.info('Using GPIO Binary');

   gpio = {
      'export': function (pin, direction) {
         var that = this;
         return exec('gpio export ' + pin + ' ' + (direction || 'out'))
         .then(function () {
            that.emit('export', pin);
            return {
               set: function () {
                  return exec('gpio write ' + pin + ' 1')
                  .then(function () {
                     that.emit('set', pin);
                  });
               },
               unset: function () {
                  return exec('gpio write ' + pin + ' 0')
                  .then(function () {
                     that.emit('unset', pin);
                  });
               },
               unexport: function () {
                  return exec('gpio unexport ' + pin)
                  .then(function () {
                     that.emit('unexport', pin);
                  });
               },
               read: function () {
                  return exec('gpio read ' + pin)
                  .then(function (stdout, stderr) {
                     return Number(stdout) === 1;
                  });
               }
            };
         });
      }
   };
}
else if (process.env.USE_GPIO_PIN) {
   var wpi = require('node-wiringpi');

   logger.info('gpio-pin');

   gpio = {
      'export': function (pin, direction) {
         var that = this;
         direction = direction || 'out';

         return Q.fcall(function () {
            wpi.pin_mode(pin, direction == 'out' ? wpi.PIN_MODE.OUTPUT : wpi.PIN_MODE.INPUT);
         })
         .then(function (gpioPin) {
            that.emit('export', pin);
            return {
               set: function () {
                  return Q.fcall(function () {
                    wpi.digital_write(pin, wpi.WRITE.HIGH);
                  })
                  .then(function () {
                     that.emit('set', pin);
                  });
               },
               unset: function () {
                  return Q.fcall(function () {
                     wpi.digital_write(pin, wpi.WRITE.LOW);
                  })
                  .then(function () {
                     that.emit('unset', pin);
                  });
               },
               unexport: function () {
                  return Q.fcall(function() {
                     that.emit('unexport', pin);
                  });
               },
               read: function () {
                  return Q.fcall(function() {
                     return 0;
                  });
               }
            };
         });
      }
   };
}
else if (process.env.NODE_ENV !== 'production') {
   logger.info('Using DEBUG GPIO');

   gpio = {
      'export': function (pin, direction) {
         logger.info('Exporting ' + pin + ' ' + direction);
         var that = this;

         return Q.fcall(function () {
            that.emit('export', pin);

            var action = function (name) {
               return function () {
                 return Q.fcall(function () {
                     that.emit(name, pin);
                     logger.info('GPIO ' + pin + ' ' + name);
                  });
               };
            };

            return {
               set: action('set'),
               unset: action('unset'),
               unexport: action('unexport'),
               read: action('read')
            };
         });
      }
   };
}

var GPIO = function () { };
util.inherits(GPIO, EventEmitter);
GPIO.prototype.export = gpio.export;
module.exports = new GPIO();